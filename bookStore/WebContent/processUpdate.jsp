...
<body>
	<%
		...
		String filename="";
		String readFolder = "c:/upload"; // 사용자가 사진을 등록하면 여기에 저장되고
	...
	<%
		...
		if (rs.next()) {
			if (fileName != null){
				sql = "update book set p_name=?, p_author=?, p_unitPrice=?, p_unitsInStrock=?, p_description=?, p_state=?, p_fileName=?";
				pstmt = conn.prepareStatement(sql);
				...
				sql = "update book set p_name=?, p_author=?, p_unitPrice=?, p_unitsInStrock=?, p_description=?, p_state=?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, name);
				pstmt.setString(2, author);
				pstmt.setInt(3, price);
